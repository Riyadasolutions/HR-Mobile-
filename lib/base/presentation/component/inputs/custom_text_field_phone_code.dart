...
class CustomTextFieldPhoneCode extends StatefulWidget {
  ...
  const CustomTextFieldPhoneCode({
    ...
    this.invalidNumberMessage = LocaleKeys.msgInvalidMobile,
    // this.cursorHeight,
...
class _CustomTextFieldPhoneCodeState extends State<CustomTextFieldPhoneCode> {
  ...
  void initState() {
    ...
    if(widget.initialCountry!=null){
      ...
      // parse initial value
      // _selectedCountry = countries.firstWhere((country) => number.startsWionUpdateMobileCountryCodeth(country.dialCode), orElse: () => _countryList.first);
      _selectedCountry = _countryList.firstWhere((item) => item.dialCode == (widget.initialDialCode ?? '255'), orElse: () => _countryList.first);
      ...
      children: [
                ...
                child: RichText(
                  text: TextSpan(
                    children: [
                      ...
                      TextSpan(text:  widget.isRequired?null:' (${tr(LocaleKeys.optional)})', style: const TextStyle().semiBoldStyle(fontSize: 12).customColor( AppColor.hintColor.themeColor )),
                    ],
                  ),
                ),
              ),
        ...
        Directionality(
          ...
          child: CustomTextField(
            ...
            prefixWidget: widget.onUpdate==null?null:Container(
              width: 60.w,
              margin: EdgeInsets.all(kFormPaddingAllLarge.w),
            ...
            validateFunc: (value) {
              ...
              if (!widget.disableLengthCheck && value != null) {
                return value.length >= _selectedCountry.minLength && value.length <= _selectedCountry.maxLength ? null : widget.invalidNumberMessage?.tr();
              }
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              if(widget.showCountryCode)...[
                FittedBox(child: Text('+${_selectedCountry.dialCode}', style: widget.dropdownTextStyle??const TextStyle().regularStyle(fontSize: 14).colorHint())),
