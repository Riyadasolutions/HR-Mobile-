...
class ShrinkSlideSideMenuState extends SideMenuState {
      ...
      child: Stack(
        ...
        children: [
          Positioned(
            ...
            bottom: 0.0,
            width: min(size.width * 0.80, widget.maxMenuWidth),
            right: widget._inverse == 1 ? null : 0,
            child: widget.menu,
          ),
          _getCloseButton(statusBarHeight),
          _getLogo(statusBarHeight),
          AnimatedContainer(
  ...
  Widget _getChild() => _opened
      ? SafeArea(
          child: Container(
            decoration:const BoxDecoration().customColor(Colors.white.withOpacity(0.10)).radius(radius: 32).borderStyle(width: 16,color: Colors.white.withOpacity(0.10)),
            child: ClipRRect(
              borderRadius: _getBorderRadius(),
  ...
  BorderRadius _getBorderRadius() => _opened
      ? (widget.radius ?? BorderRadius.circular(32.0))
      : BorderRadius.zero;
  ...
  Matrix4 _getMatrix4(Size size) {
    if (_opened) {
      return Matrix4.identity()
        ..translate(
            min(size.width, widget.maxMenuWidth) * widget._inverse * (widget.inverse ? 0.8 : 0.9),
            (size.height * 0.1))
