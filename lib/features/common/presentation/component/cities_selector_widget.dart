...
import 'package:base_app/features/common/presentation/sheets/city_picker/city_picker_sheet.dart';
...
class CitiesSelectorWidget extends StatelessWidget {
  ...
  const CitiesSelectorWidget({super.key,
    ...
    String? iconSVG,
    bool isDark = false,
  })  : _isMultiChoice = isMultiChoice,_isSelectAllEnable = isSelectAllEnable,
  ...
  Widget build(BuildContext context) {
    return CustomButtonArrow(
       ...
       value: _selectedValue?.title ?? ((_selectedList ?? []).isNotEmpty ? (_selectedList ?? []).map((e) => e.title).toList().join(' , ') :null),
        ...
        label: _title/*??tr(LocaleKeys.city)*/,
        hint: _hint?? _title??tr(LocaleKeys.city),
  ...
  Future<void> _onPressed(
    ...
    if(_countryId == null) {
      Alerts.showSnackBar(tr(LocaleKeys.youMustSelectTheCountryFirst),alertsType: AlertsType.info);
      return;
    }
    var result = await showCityPicker(
      context,
      isMultiChoice: _isMultiChoice,
      isSelectAllEnable: _isSelectAllEnable,
