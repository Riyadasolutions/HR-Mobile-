...
run--no-sound-null:
	flutter run --no-sound-null-safety -vv
run-prd:
...
build-android-prd-abb:
	...
	flutter build appbundle --release --no-shrink --flavor production --dart-define=FLAVOR=production --obsecure --target lib/main.dart
build-android-V8:
	flutter build apk --target-platform android-arm,android-arm64,android-x64 --split-per-abi
build-android-dev-abb:
	flutter build appbundle --release --no-shrink --flavor development --dart-define=FLAVOR=development --target lib/main.dart
